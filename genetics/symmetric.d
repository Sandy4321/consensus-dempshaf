module uncertainty.genetics.symmetric;

import uncertainty.genetics.genetics;
import std.math, std.parallelism;
import std.stdio;

public class Symmetric : Genetics
{
	public
	{
			...
			int l)
		{
			...
			for (int i; i < l; i++)
			{
				...
				maximalPayoff += value < 0 ? value * -1 : value;
			}
			...
			in int amt)
		{
			...
			double payoff, choice, k;

			auto minPayoff = Symmetric.minPayoff(payoffMap);
			auto totalPayoff = Symmetric.totalPayoff(payoffMap, minPayoff);

			k = 0.5;		// Calculate epsilon value for roulette selection
			...
			while(select < amt)
			{
				...
				{
					payoff += (value - minPayoff + k) / (totalPayoff + (n * k));
					...
					if (select > 0 && selection[$ - 2] == selection[$ - 1])
						select--;
					break;
				}
			}
		...
		static int[] rouletteSelection(	ref Random rand,
										double[] payoffMap,
										int l,
										int amt)
		{
			...
			//auto minPayoff = Symmetric.minPayoff(payoffMap);
			auto totalPayoff = Symmetric.totalPayoff(payoffMap, 0);
		...
		/*
		 * valuationDistance: takes opinions and probabilities for both agents
